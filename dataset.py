...
class Data_Set(Dataset):
    def __init__(self, video_folder, resize_height=256, resize_width=256, time_steps=5, mode='train'):
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train':
            ...
            return [image[0], image[1], image[2], image[3], image[4]]  # 返回的是 I_0, I_t, I_1, t
        else :
            ...
            tf = transforms.Compose([
                lambda x: Image.open(x).convert('RGB'),  # string path= > image data
                transforms.Resize((self.original_shape[1] // 32 * 32, self.original_shape[0] // 32 * 32)),
                transforms.ToTensor()
                # transforms.Normalize(mean=[0.485, 0.456, 0.406],
                #                      std=[0.229, 0.224, 0.225])
            ])
...
def main():
    data_set = Data_Set('../data/ped2/testing/frames', resize_height=360, resize_width=360, time_steps=5, mode='train')
    ...
    [I_0, I_1, I_2, I_3, I_4] = data_set.__getitem__(0)
